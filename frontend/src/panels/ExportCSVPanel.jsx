...
export default function ExportCSVPanel() {
  ...
  const selectClass = 'w-full px-4 py-2.5 bg-surface-input border border-border-default rounded-lg text-slate-100 text-sm outline-none transition-all duration-150 focus:border-blue-500 focus:ring-3 focus:ring-blue-500/15 appearance-none bg-[url("data:image/svg+xml,%3Csvg%20xmlns=%27http://www.w3.org/2000/svg%27%20width=%2712%27%20height=%2712%27%20viewBox=%270%200%2012%2012%27%3E%3Cpath%20fill=%27%2394a3b8%27%20d=%27M6%208L1%203h10z%27/%3E%3C/svg%3E")] bg-no-repeat bg-[right_1rem_center] pr-10 cursor-pointer';

  return (
    <div className="animate-page-enter">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-[1.75rem] font-extrabold bg-gradient-to-r from-slate-100 to-blue-300 bg-clip-text text-transparent">📤 Export to CSV</h1>
        <p className="text-slate-400 mt-1 text-sm">Export sensor data from any device as CSV file</p>
      </div>
      ...
      {/* Config Card */}
      <div className="bg-surface-card border border-white/5 rounded-xl p-6 mb-6">
        <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/5">
          <h3 className="text-base font-bold text-slate-100">Export Configuration</h3>
        </div>

        {fetchingDevices ? (
          <div className="flex items-center justify-center py-8 text-slate-500 text-sm gap-3">
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wide" htmlFor="export-device">Select Device</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-2 uppercase tracking-wide" htmlFor="export-limit">Data Limit</label>
            ...
            <div className="flex gap-3">
              <button onClick={handlePreview} disabled={loading || !selectedDevice} id="preview-btn" className="px-5 py-2.5 bg-surface-elevated border border-border-default rounded-lg text-sm font-semibold text-slate-100 hover:bg-surface-card-hover transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
                {loading ? <><div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> Loading...</> : '👁 Preview Data'}
              </button>
              {preview && preview.data.length > 0 && (
                <button onClick={handleExport} id="export-btn" className="px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-500 rounded-lg text-sm font-semibold text-white hover:shadow-[0_0_20px_rgba(34,197,94,0.3)] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150">
                  📥 Download CSV
                </button>
              )}
            </div>
      ...
      {/* Preview */}
      {preview && (
        <div className="bg-surface-card border border-white/5 rounded-xl p-6 mt-6">
          <div className="flex items-center justify-between mb-5 pb-4 border-b border-white/5">
            <h3 className="text-base font-bold text-slate-100">📋 Data Preview — {preview.device?.name}</h3>
          ...
          {preview.data.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <div className="text-lg font-semibold text-slate-400">No data available</div>
            </div>
          ) : (
            <div className="overflow-x-auto rounded-xl border border-white/5 max-h-[400px] overflow-y-auto">
              <table className="w-full border-collapse text-sm">
                <thead className="bg-surface-elevated sticky top-0">
                  <tr>
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 border-b border-border-default">ID</th>
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 border-b border-border-default">Payload</th>
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 border-b border-border-default">Timestamp</th>
                  </tr>
                </thead>
                <tbody>
                  {preview.data.slice(0, 20).map((item) => (
                    <tr key={item.id} className="transition-colors duration-150 hover:bg-blue-500/[0.04] border-b border-white/5 last:border-b-0">
                      <td className="px-4 py-3.5">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20">#{item.id}</span>
                      </td>
                      <td className="px-4 py-3.5">
                        <div className="bg-surface-input border border-white/5 rounded-lg px-3 py-2 font-mono text-xs text-cyan-300 overflow-x-auto whitespace-pre-wrap break-all max-h-[120px] overflow-y-auto">
                          {JSON.stringify(item.payload, null, 2)}
                        </div>
                      </td>
                      <td className="px-4 py-3.5 text-xs text-slate-500">{formatDate(item.createdAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
